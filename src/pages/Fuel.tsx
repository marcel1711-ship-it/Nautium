...
export const Fuel: React.FC<FuelProps> = ({ onNavigate, params }) => {
  ...
  const handleExportPDF = () => {
    ...
    const rows = filteredLog.map(entry => {
      ...
      const pdfIsWaste = res && (res.resource_type === 'grey_water' || res.resource_type === 'black_water');
      const pdfLabel = pdfIsWaste
        ? (isRefill ? '↑ Level Update' : '↓ Pump Out')
      ...
      return `
        <tr>
          ...
          <td class="${isRefill ? 'refill' : 'consume'}">${pdfLabel}</td>
                      ...
                      <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${
                            isWaste
                              ? (pct > 80 ? 'bg-red-500' : pct > 60 ? 'bg-amber-400' : colors.bar)
                              : (isLow ? colors.low : colors.bar)
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                        ...
                        <>
                          {lastConsumption && (
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                              <TrendingDown className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />
                              <span>{t('fuel.lastPumpOut')} <strong className="text-gray-700">−{lastConsumption.quantity.toLocaleString()} {resource.unit}</strong> — {new Date(lastConsumption.log_date).toLocaleDateString('en-GB')}</span>
                            </div>
                          )}
                        </>
                      ) : (
                      ...
                      {isWaste ? (
                        <>
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl text-sm font-semibold transition-all border border-blue-200"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center justify-center gap-1.5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-sm font-semibold transition-all border border-slate-200"
                          >
          ...
          {filteredLog.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
              {filteredLog.map(entry => {
                const res = resources.find(r => r.id === entry.resource_id);
                const isRefill = entry.entry_type === 'refill';
                const entryIsWaste = res && (res.resource_type === 'grey_water' || res.resource_type === 'black_water');
                const badgeLabel = entryIsWaste
                  ? (isRefill ? '↑ Level Update' : '↓ Pump Out')
                  : (isRefill ? '+ Refill' : '− Consumption');
                const badgeClass = entryIsWaste
                  ? (isRefill ? 'bg-slate-100 text-slate-700' : 'bg-blue-100 text-blue-700')
                  : (isRefill ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700');
                const iconBg = entryIsWaste
                  ? (isRefill ? 'bg-slate-100' : 'bg-blue-100')
                  : (isRefill ? 'bg-green-100' : 'bg-orange-100');
                const iconColor = entryIsWaste
                  ? (isRefill ? 'text-slate-600' : 'text-blue-600')
                  : (isRefill ? 'text-green-600' : 'text-orange-500');
                const qtyColor = entryIsWaste
                  ? (isRefill ? 'text-slate-700' : 'text-blue-700')
                  : (isRefill ? 'text-green-700' : 'text-orange-600');
                return (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="text-right flex-shrink-0">
                            <p className={`font-bold text-base ${qtyColor}`}>
